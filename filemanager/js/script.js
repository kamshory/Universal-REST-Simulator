...
function initPreviewImageUpload() {
	$(document).on("click", "#image-list li img", function () {
		...
		$(".preview-image").animate(
			{ width: width2, height: height2, left: endX, top: endY },
			200
		);
		$(".preview-image, .ui-dialog, .ui-widget-overlay").click(function () {
			show = false;
			$(".preview-image").animate(
				{ width: width, height: height, left: startX, top: startY },
				200,
				function () {
					if (!show) {
						$(".preview-image").remove();
...
function updateToolbarStatus() {
	...
	if ($(".fileid:checked").length == 0) {
		...
		$(".toolbar-inner li a img.compress")
			.parent()
			.parent()
			.addClass("inactive");
		$(".toolbar-inner li a img.permission")
			.parent()
			.parent()
			.addClass("inactive");
	} else {
		$(".toolbar-inner li a img.uncheck")
			.parent()
			.parent()
			.removeClass("inactive");
		...
		$(".toolbar-inner li a img.delete")
			.parent()
			.parent()
			.removeClass("inactive");
		$(".toolbar-inner li a img.rename")
			.parent()
			.parent()
			.removeClass("inactive");
		$(".toolbar-inner li a img.compress")
	...
	if (
		$('.row-data-file[data-file-type="application/zip"]').find(
			"input[type=checkbox]:checked"
		).length
	) {
		$(".toolbar-inner li a img.extract")
			.parent()
			.parent()
			.removeClass("inactive");
	} else {
...
function jqAlert(msg, title, width, height) {
	$("#mb-area").remove();
	$("body").append(
		'<div id="mb-area" style="display:none;"><div id="message-box-dialog"><div id="message-box-dialog-inner"></div></div></div>'
	);
...
function contextMenu(selector, menu) {
	$(selector).on("contextmenu", function (e) {
		...
		for (var i in menu) {
			...
			if (
				!$(selector).hasClass("row-data-file") &&
				!$(selector).hasClass("row-data-dir")
			) {
				if (
					clipboardfile.operation == "" &&
					(classname == "empty-clipboard" || classname == "paste")
				) {
					continue;
				}
				if (
				...
				if (
					$(".fileid:checked").length == $(".fileid").length &&
					classname == "check"
				) {
					continue;
				}
			...
			html +=
				'<li class="file-function file-function-' +
				classname +
				'"><a href="' +
				menu[i]["linkurl"] +
				'">' +
...
function initContextMenuFileArea() {
	var cm = [
		...
		{
			caption: "Cut Selected File",
			linkurl: "javascript:cutSelectedFile()",
			classname: "cut",
		},
		{
			caption: "Move Selected File",
			linkurl: "javascript:moveSelectedFile()",
			classname: "move",
		},
		{
			caption: "Delete Selected File",
			linkurl: "javascript:deleteSelectedFile()",
			classname: "delete",
		},
		{
		...
		},
		{
			...
			linkurl: "javascript:changePermission()",
			classname: "permission",
		},
		{
			caption: "Paste File",
			linkurl: "javascript:pasteFile()",
			classname: "paste",
		},
		{
			caption: "Create New File",
			linkurl: "javascript:createFile()",
			classname: "createfile",
		},
		{
			caption: "Create New Directory",
			linkurl: "javascript:createDirectory()",
			classname: "createdir",
		},
		{
			caption: "Up Directory",
			linkurl: "javascript:goToUpDir()",
			classname: "up",
		},
		{
			caption: "Refresh File List",
			linkurl: "javascript:refreshList()",
			classname: "refresh",
		},
		{
			caption: "Change View Type",
			linkurl: "javascript:thumbnail()",
			classname: "view",
		},
		{
			caption: "Upload File",
			linkurl: "javascript:uploadFile()",
			classname: "upload",
		},
		{
			caption: "Check All",
			linkurl: "javascript:selectAll(1)",
			classname: "check",
		},
		{
			caption: "Uncheck All",
			linkurl: "javascript:selectAll(0)",
			classname: "uncheck",
		},
		{
			caption: "Empty Clipboard",
			linkurl: "javascript:emptyClipboard()",
...
function contextMenuListFile(filetype, filepath, fileurl, attr) {
	...
	if (filetype.indexOf("image") == 0) {
		...
		cm = [
			{
				caption: "Select File",
				linkurl: "javascript:selectFile('" + fileurl2 + "')",
				classname: "select",
			},
			{
				caption: "Copy File",
				linkurl: "javascript:copyFile('" + filepath + "')",
				classname: "copy",
			},
			{
				caption: "Cut File",
				linkurl: "javascript:cutFile('" + filepath + "')",
				classname: "cut",
			},
			{
				caption: "Rename File",
				linkurl: "javascript:renameFile('" + filepath + "')",
				classname: "rename",
			},
			{
				caption: "Move File",
				linkurl: "javascript:moveFile('" + filepath + "')",
				classname: "move",
			},
			{
				caption: "Delete File",
				linkurl: "javascript:deleteFile('" + filepath + "')",
				classname: "delete",
			},
			{
				caption: "Preview Image",
				linkurl:
					"javascript:previewFile('" +
					fileurl2 +
					"', " +
					width +
					", " +
			...
			{
				caption: "Edit Image",
				linkurl: "javascript:editImage('" + filepath + "')",
				classname: "edit-image",
			},
			{
				caption: "Compress File",
				linkurl: "javascript:compressFile('" + filepath + "')",
				classname: "compress",
			},
			{
				caption: "Set Permission",
				linkurl: "javascript:changePermission('" + filepath + "')",
				classname: "permission",
			},
			{
				caption: "Download File",
				linkurl: fileurl2 + '" target="_blank',
				classname: "download",
			},
			{
				caption: "Force Download File",
				linkurl: "javascript:forceDownloadFile('" + filepath + "')",
				classname: "download",
			},
			{
				caption: "Image Properties",
				linkurl: "javascript:propertyImage('" + filepath + "')",
				classname: "property",
			},
		];
		...
		cm = [
			...
			{
				caption: "Play Video",
				linkurl: "javascript:playVideo('" + fileurl2 + "', 'html5')",
				classname: "play",
			},
			{
				caption: "Compress File",
			...
			},
			{
				...
				linkurl: "javascript:propertyVideo('" + filepath + "')",
				classname: "property",
			},
		];
		...
		cm = [
			{
				caption: "Select File",
				linkurl: "javascript:selectFile('" + fileurl + "')",
				classname: "select",
			},
			{
				caption: "Copy File",
			...
			{
				caption: "Play Audio",
				linkurl: "javascript:playAudio('" + fileurl + "', 'html5')",
				classname: "play",
			},
			{
				caption: "Compress File",
			...
			},
			{
				...
				linkurl: "javascript:propertyFile('" + filepath + "')",
				classname: "property",
			},
		];
		...
		cm = [
			...
			{
				caption: "Read PDF Document",
				linkurl: "javascript:previewPDF('" + fileurl + "')",
				classname: "preview",
			},
			{
				caption: "Compress File",
		...
		cm = [
			...
			{
				caption: "View Shock Wave",
				linkurl:
					"javascript:previewSWF('" +
					fileurl +
					"', " +
					width +
					", " +
		...
		cm = [
			...
			},
			{
				...
				linkurl: "javascript:extractFile('" + filepath + "')",
				classname: "extract",
			},
			{
				caption: "Set Permission",
		...
		cm = [
			...
			},
			{
				...
				linkurl: "javascript:editFile('" + filepath + "')",
				classname: "edit",
			},
			{
				caption: "Edit Code",
				linkurl:
					"code-editor.php?filepath=" +
	...
	cm = [
		{
			...
			linkurl: 'javascript:;" onClick="return openDir(\'' + filepath + "')",
			classname: "open",
		},
		{
		...
		},
		{
			...
			linkurl: "javascript:renameFile('" + filepath + "', true)",
			classname: "rename",
		},
		{
			caption: "Cut Directory",
			linkurl: "javascript:cutFile('" + filepath + "')",
			classname: "cut",
		},
		{
			caption: "Move Directory",
			linkurl: "javascript:moveFile('" + filepath + "', true)",
			classname: "move",
		},
		{
			caption: "Delete Directory",
			linkurl: "javascript:deleteDirectory('" + filepath + "')",
			classname: "delete",
		},
		{
			caption: "Compress Directory",
			linkurl: "javascript:compressFile('" + filepath + "')",
			classname: "compress",
		},
		{
		...
		},
		{
			...
			linkurl: "javascript:propertyDir('" + filepath + "')",
			classname: "property",
		},
	];
	return cm;
...
function forceDownloadFile(filepath) {
	window.open(
		"tool-download-file.php?filepath=" + encodeURIComponent(filepath)
	);
}
...
function previewFile(url, width, height, fullsize, frommenu) {
	...
	var url2 = 'tool-preview-file.php?filepath=' + relative2absolute(url);

	if (fullsize) {
		var obimg = $('<img>');
		var onclk = "previewFile('" + encodeURI(url) + "', " + width + ", " + height + ", false)";
		obimg.attr({ 'src': url2, 'width': w, 'height': h, 'title': bn, 'onclick': onclk });
		obimg.addClass('image2zoomout');
		html = obimg[0].outerHTML;
	} else {
		if (width > 500) {
			w = 500;
			h = (height / width) * w;
		...
		var obimg = $('<img>');
		var onclk = "previewFile('" + encodeURI(url) + "', " + width + ", " + height + ", true)";
		obimg.attr({ 'src': url2, 'width': w, 'height': h, 'title': bn, 'onclick': onclk });
		obimg.addClass('image2zoomout');
		html = obimg[0].outerHTML;
	}
	...
	var toolHTML = "";
	prevHTML +=
		'<div class="nav-thumb nav-thum-prev" style="top:' +
		navtop +
		"px;left:" +
		navleft +
		'px;" onclick="previewFile(\'' +
		prevThumb["url"] +
		"', " +
		prevThumb["width"] +
		", " +
		...
		');">&lt;</div>';
	nextHTML +=
		'<div class="nav-thumb nav-thum-next" style="top:' +
		navtop +
		"px;left:" +
		navright +
		'px;" onclick="previewFile(\'' +
		...
		');">&gt;</div>';
	toolHTML +=
		'<div class="tool-thumb" style="top:' +
		tooltop +
		"px;left:" +
		toolright +
		'px;">' +
		'<a href="javascript:;" onclick="selectFileIndex(\'' +
		url +
		'\');" title="Select Image"><img src="style/images/trans16.gif" class="select" border="0"></a> ' +
		'<a href="javascript:;" onclick="window.open(\'' +
		url +
		'\');" title="Download Image"><img src="style/images/trans16.gif" class="download" border="0"></a> ' +
		'<a href="javascript:;" onclick="editImage(\'' +
		relative2absolute(url) +
		'\');closeOverlayDialog();" title="Edit Image"><img src="style/images/trans16.gif" class="edit-image" border="0"></a> ' +
		'<a href="javascript:;" onclick="propertyImage(\'' +
		relative2absolute(url) +
		'\');" title="Image Properties"><img src="style/images/trans16.gif" class="property" border="0"></a> ' +
		"</div>";
	...
	if (frommenu && togglethumb) {
		...
		if (obj.length > 0) {
			...
			var bgurl =
				"url(tool-thumb-image.php?filepath=" +
				encodeURIComponent(relative2absolute(url)) +
				")";
			$("<img />")
				.addClass("anim-preview")
				.attr("src", url)
				.css({
			...
			$(".anim-preview").animate(
				{ left: lto, top: tto, width: w, height: h },
				200,
				function () {
					$(".anim-preview").remove();
					...
					$(".image2zoomout, .image2zoomin").on("mouseover", function () {
						...
						$('li[data-file-url="' + imgsrc + '"] .thumbitem').addClass(
							"thumbitem-selected"
						);
					});
					...
					$(".image2zoomout, .image2zoomin").on("mouseout", function () {
						var imgsrc = $(this).attr("src");
						$('li[data-file-url="' + imgsrc + '"] .thumbitem').removeClass(
							"thumbitem-selected"
						);
					});
				}
...
function preloadImage() {
	$(document).on(
		"hover",
		'.row-data-file[data-file-type*="image"]',
		function () {
...
function playVideo(url, type) {
	...
	if (type == "iframe")
		html =
			'<iframe frameborder="0" hspace="0" vspace="0" marginheight="0" marginwidth="0" scrolling="auto" src="' +
			url +
			'" width="500" height="375" /></ifame>';
	else if (type == "html5")
		html =
			'<video src="' +
			url +
			'" poster="style/images/movie.png" controls="true" width="500" height="375"><object width="500" height="375" type="application/x-shockwave-flash" data="moxieplayer.swf"><param name="url" value="' +
			url +
			'"><param name="src" value="moxieplayer.swf"><param name="allowfullscreen" value="true"><param name="allowscriptaccess" value="true"><param name="autoplay" value="true"><param name="flashvars" value="url=' +
			url +
			'&amp;poster=style/images/movie.png&amp;autoplay=true"></object></video>';
	else if (type == "moxie")
		html =
			'<object width="500" height="375" type="application/x-shockwave-flash" data="moxieplayer.swf"><param name="url" value="' +
			url +
			'"><param name="src" value="moxieplayer.swf"><param name="allowfullscreen" value="true"><param name="allowscriptaccess" value="true"><param name="autoplay" value="true"><param name="flashvars" value="url=' +
			url +
			'&amp;poster=style/images/movie.png&amp;autoplay=true"></object>';
	else
		html =
			'<object width="500" height="375" data="' +
			url +
			'" type="application/x-mplayer2"><param name="url" value="' +
			url +
			'" /></object>';
...
function playAudio(url, type) {
	...
	if (type == "iframe")
		html =
			'<iframe frameborder="0" hspace="0" vspace="0" marginheight="0" marginwidth="0" scrolling="auto" src="' +
			url +
			'" width="320" height="50" /></ifame>';
	else if (type == "html5")
		html =
			'<video src="' +
			url +
			'" poster="audio.jpg" controls="true" width="320" height="50"><object width="320" height="50" data="' +
			url +
			'" type="application/x-mplayer2"><param name="url" value="' +
			url +
			'" /></object></video>';
		...
		html =
			'<object width="320" height="50" type="application/x-shockwave-flash" data="moxieplayer.swf"><param name="url" value="' +
			url +
			'"><param name="src" value="moxieplayer.swf"><param name="allowfullscreen" value="true"><param name="allowscriptaccess" value="true"><param name="autoplay" value="true"><param name="flashvars" value="url=' +
			url +
			'&amp;poster=style/images/audio.png&amp;autoplay=true"></object>';
	else
		html =
			'<object width="320" height="50" data="' +
			url +
			'" type="application/x-mplayer2"><param name="url" value="' +
			url +
			'" /></object>';
...
function previewPDF(url) {
	var html =
		'<embed src="' +
		url +
		'#toolbar=0&amp;navpanes=0&amp;scrollbar=0" width="720" height="400">';
...
function previewSWF(url, width, height) {
	var html =
		...
		'" height="' +
		height +
		'" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0"><param name="src" value="' +
		url +
		'"><embed src="' +
		url +
		'" width="' +
	...
	$.get("tool-property-file.php", { filepath: filepath }, function (answer) {
		...
		if (mime == "application/zip") {
			$(".ui-dialog-buttonset").prepend(
				'<button aria-disabled="false" role="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" type="button"><span class="ui-button-text">Show Content</span></button>'
			);
			$(".ui-dialog-buttonset")
				.find("button:first")
				.attr("onclick", "showZipContent('" + filepath + "')");
		}
	});
}
...
function propertyDir(filepath) {
	...
	$.get(
		"tool-property-file.php",
		{ filepath: filepath, type: "directory" },
		function (answer) {
			$("#common-dialog-inner").html(answer);
		}
	);
				...
				OK: function () {
					...
					if (nn == "") {
						...
						if (oe.toString() != ne.toString() && !isdir) {
							if (
								!confirm(
									"The new file extension is different with old one.\n" +
						...
						$.post(
							"tool-file-operation.php?option=renamefile",
							...
							function (answer) {
								if (answer == "SUCCESS") {
									...
									jqAlert(
										nn + " already exists. Please type another name.",
										"Invalid Name"
									);
								} else if (answer == "READONLY") {
									jqAlert(
										"The operation was disabled on read-only mode.",
										"Read-Only"
									);
								} else if (answer == "FORBIDDENEXT") {
									jqAlert(
										"The operation was aborted because this file name extension is forbidden. Please use another file name extension.",
										"Forbidden Extension"
									);
								}
							}
		...
		buttons: {
			OK: function () {
				$(".ui-dialog-buttonpane").append(
					'<div class="wait-status">Plase wait...</div>'
				);
				...
				$.post(
					"tool-file-operation.php?option=compressfile",
					{ "sourcepath[]": sf, targetpath: tf },
					function (answer) {
						if (answer == "CONFLICT") {
							jqAlert(
								filepath + " already exists. Please enter another name.",
								"Invalid Name"
							);
							...
							jqAlert(
								...
								"ZipArchive class not exists. Please verify that php_zip extension is available on this server."
							);
						} else if (answer == "READONLY") {
							jqAlert(
								"The operation was disabled on read-only mode.",
						...
						$(".ui-dialog-buttonset button:first").attr(
							"aria-disabled",
							"false"
						);
					}
...
function compressSelectedFile() {
	...
	if (chk) {
		...
		html =
			...
			"</tr>" +
			"</table>" +
			'<div></div><div>File to be compressed:</div><div class="seleted-file-list">' +
			file2compress +
			"</div></div>";
		$("#common-dialog-inner").html(html);
		...
		var num2 = 1;
		do {
			if (i > 1) dir = val + "-" + i + ".zip";
			else dir = val + ".zip";
			num1 = $('.file-area .row-data-dir[data-file-name="' + dir + '"]').length;
			num2 = $(
				'.file-area .row-data-file[data-file-name="' + dir + '"]'
			).length;
			i++;
		} while (num1 || num2);
		...
		var endPos = startPos + name.length;
		setInputSelection(
			document.getElementById("fftargetpath"),
			startPos,
			endPos
		);
	} else {
...
function moveSelectedFile() {
	...
	if (chk) {
		...
		html =
			...
			"</tr>" +
			"</table>" +
			'<div></div><div>File to be moved:</div><div class="seleted-file-list">' +
			file2move +
			"</div></div>";
		$("#common-dialog-inner").html(html);
		$("#fftargetdir").focus();
	...
	$("#common-dialog").dialog({
		...
		buttons: {
			OK: function () {
				...
				var targetdir = $("#fftargetdir").val();
				$.post(
					"tool-file-operation.php?option=extractfile",
...
function createFile() {
	...
	$("#common-dialog").dialog({
		...
		buttons: {
			OK: function () {
				...
				var dn = $("#ffname").val();
				$.post(
					"tool-file-operation.php?option=createfile",
					{ location: dl, name: dn },
					function (answer) {
						if (answer == "EXIST") {
							jqAlert(
								dl + "/" + dn + " already exists. Please type another name."
							);
							$("#ffname").select();
						} else if (answer == "SUCCESS") {
							openDir(dl);
							...
							jqAlert(
								"Creating file was aborted because this file name extension is forbidden. Please use another file name extension.",
								"Forbidden Extension"
							);
						} else if (answer == "READONLY") {
							jqAlert(
	...
	var val = "new-file";
	var i = 1;
	var dir = val + "." + defaultExt;
	var num1 = 1;
	var num2 = 1;
	do {
		if (i > 1) dir = val + "-" + i + "." + defaultExt;
		else dir = val + "." + defaultExt;
		num1 = $('.file-area .row-data-dir[data-file-name="' + dir + '"]').length;
	...
	$("#common-dialog").dialog({
		...
		buttons: {
			OK: function () {
				...
				var dn = $("#ffname").val();
				$.post(
					"tool-file-operation.php?option=createdir",
					{ location: dl, name: dn },
					function (answer) {
						if (answer == "EXIST") {
							jqAlert(
...
function saveFile(filepath, filecontent, callback) {
	if (filepath == "") {
		...
		$(".ui-dialog-buttonset button:first").attr("aria-disabled", "true");
		$.post(
			"tool-edit-file.php?option=savefile",
			...
			function (answer) {
				if (answer == "READONLY") {
					...
					jqAlert(
						"Saving was aborted because this file is read-only. You should to change permission of this file first.",
						"Read-Only"
					);
				} else if (answer == "ISDIR") {
					jqAlert(
						"Saving was aborted because this file name is similiar to a directory name. You should to change file name first.",
						"Invalid File Name"
					);
				} else if (answer == "FORBIDDENEXT") {
					jqAlert(
						"Saving was aborted because this file name extension is forbidden. Please use another file name extension to save it.",
						"Forbidden Extension"
					);
	...
	$("#common-dialog").dialog({
		...
		buttons: {
			...
			"Save and Close": function () {
				saveFile($("#filepath").val(), $("#filecontent").val(), function () {
					$("#common-dialog").dialog("destroy");
				});
			},
		...
		resize: function (event, ui) {
			$("#filecontent").css({ height: ui.size.height - 160 + "px" });
		},
	});
	$(".ui-dialog-titlebar-close").remove();
	$.get(
		"tool-edit-file.php",
		{ option: "openfile", filepath: filepath },
		function (answer) {
			$("#common-dialog-inner").html(answer);
			cnt1 = $("#filecontent").val();
		}
	...
	$("#common-dialog").dialog({
		...
		buttons: {
			OK: function () {
				...
				args = "file[]=" + filepath;
				$.post(
					"tool-file-operation.php?option=deletefile",
					{ postdata: args },
					function (answer) {
						if (answer == "SUCCESS") {
							openDir(dl);
...
function deleteDirectory(filepath) {
	...
	var html =
		"" +
		"<div>Are you sure to delete this directory including its content:<br />" +
		filepath +
		"</div>";
	$("#common-dialog-inner").html(html);
	$(".ui-dialog-buttonset button:last").focus();
...
function deleteSelectedFile() {
	...
	if (chk) {
		...
		html =
			'<div>Are you sure to delete file/s:</div><div class="seleted-file-list">' +
			file2del +
			"</div></div>";
		$("#common-dialog-inner").html(html);
		$(".ui-dialog-buttonset button:last").focus();
	...
	$("#common-dialog").dialog({
		...
		buttons: {
			"Get File": function () {
				...
				$.post(
					"tool-file-operation.php?option=transferfile",
...
function showClipboard() {
	...
	html =
		...
		"&quot;; " +
		"Number of File: " +
		clipboardfile.content.length +
		"</div>" +
		'<div class="seleted-file-list">' +
		clipboardfile.content.join("<br />") +
		"</div>" +
		"";
	$("#common-dialog-inner").html(html);
}
function changePermission(filepath) {
	...
	$("#common-dialog-inner").html(
		'<table width="100%" border="0" cellspacing="1" cellpadding="2" class="row-table file-table permission-table">' +
		"<thead>" +
		"  <tr>" +
		"    <td>User</td>" +
		'    <td width="25%">Read</td>' +
		...
		buttons: {
			Change: function () {
				if (data.length == 0) {
					...
					$.post(
						"tool-file-operation.php?option=change-perms",
	...
	if (!filepath) {
		...
		$.get(
			"tool-file-operation.php",
			{ option: "get-perms", filepath: filepath },
			function (answer) {
...
function editImage(fp) {
	...
	$.get("tool-image-editor-form.php", { filepath: fp }, function (answer) {
		...
		$("#image-editor-layer").show(
			"size",
			options,
			500,
			callbackShowImageEditor
		);
		initImageEditorForm();
	});
}
...
function initImageEditorForm() {
	...
	$(document).on("change", "#newwidth", function () {
		...
		$("#image2edit").css({
			width: $("#newwidth").val() + "px",
			height: $("#newheight").val() + "px",
		});
...
function previewImageEdit() {
	...
	var html =
		'<img src="tool-image-editor-thumbnail.php?filepath=' +
		encodeURIComponent(filepath) +
		"&flipv=" +
		flipv +
		"&fliph=" +
...
function saveImage() {
	if (confirm("Are you sure to save this state and replace current file?")) {
		...
		var args =
			...
			crop;
		$.post(
			"tool-image-editor-thumbnail.php",
			{ postdata: args },
			function (answer) {
				if (answer == "SUCCESS") {
					$.get(
...
function initDropable() {
	...
	$(".file-list .row-data-dir").droppable({
		...
		drop: function (event, ui) {
			var curlocation =
				ui.draggable.attr("data-file-location") +
				"/" +
				ui.draggable.attr("data-file-name");
			var targetdir =
			...
			skipondrop = true;
			ui.draggable.hide("scale", { percent: 0 }, 300, function () {
...
function openDirTree() {
	...
	for (i in dirs) {
		...
		if (
			!$("#directory-container").find('a[onclick~="' + _curdir + '"]').length
		) {
			var _child =
				'<ul><li class="row-data-dir dir-control" data-file-path="' +
				_curdir +
				'" data-file-name="' +
				dirs[i] +
				'" data-file-location="' +
				_parentdir +
				'"><a href="javascript:;" onclick="return openDir(\'' +
				_curdir +
				"')\">" +
				dirs[i] +
				"</a></li></ul>";
			$("#directory-container")
				.find('[data-file-path="' + _parentdir + '"]')
				.append(_child);
		}
		_parentdir = _curdir;
	}
		...
		$.get("tool-load-file.php", arg, function (answer) {
			...
			try {
				...
				if (
					...
					try {
						$("#control-fileid").attr("checked", "checked");
					} catch (e) { }
				} else {
					try {
						$("#control-fileid").removeAttr("checked");
					} catch (e) { }
				}
			} catch (e) { }
			initContextMenuFile();
			...
			$("#directory-container .basedir ul li")
				.removeClass("dir-open")
				.addClass("dir-normal");
			$(
				'#directory-container .basedir ul li[data-file-path="' + filepath + '"]'
			).addClass("dir-open");
		});
		var pth = "";
...
function initPermission() {
	$(document).on("click", ".permission-info", function () {
		var perms = $(this).text() + "";
		try {
			$("#perms-dialog").dialog("destroy");
		} catch (e) { }
		var perms_obj = showPermission(perms);
		var p_key;
		for (p_key in perms_obj) {
			$("." + p_key).text(
				parseInt(perms_obj[p_key]) == 1 ? "Allowed" : "Disallowed"
			);
		}
		$("#perms-dialog").dialog({
			resizable: false,
...
function normalizeTable() {
	$(".file-result-table tbody tr").each(function (index, element) {
		...
		tr.attr("data-file-name-lower", name + path);
	});
}
function filterFile(name) {
	...
	if (name == "") {
		...
		$('.file-result-table tbody tr[data-file-name-lower*="' + name + '"]').css({
			display: "",
		});
	}
...
function initEXIF() {
	$(document).on("click", ".capture-info", function () {
		...
		$("#exif-dialog").remove();
		$("#dialogs").append(
			'<div id="exif-dialog"><div id="exif-dialog-inner"></div></div>'
		);
		var jsdata = eval(decodeURIComponent($(this).attr("data-exif")));
		var key;
		var obj = jsdata[0];
		var html =
			'<table width="100%" border="0" cellspacing="1" cellpadding="2" class="row-table exif-table">';
		html += "<tbody>";
		for (key in obj) {
			html +=
				'<tr><td width="50%">' + key + "</td><td>" + obj[key] + "</td></tr>";
		}
		html += "</tbody></table>";
...
function FileSelectHandler(e) {
	...
	html +=
		'<div class="progressbar" style="margin:5px 0px;height:10px; background-color:#EEEEEE;"><div class="progressbar-inner" style="height:10px;background-color:rgb(23, 96, 125);width:0%;"></div></div>';
	...
	xhr.open(
		"POST",
		"tool-upload-file-multi.php?targetdir=" +
